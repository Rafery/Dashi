...
public class DBConnection {
	...
	private ResultSet executeFetchStatement(String query) {
		...
		try {
			Statement stmt = conn.createStatement();
			System.out.println("\nDBConnection executing query:\n" + query);
			return stmt.executeQuery(query);// execute hui fan hui sqlresult,
											// while execute update bu hui, see
											// DBYelpImport or above
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public JSONObject getRestaurantsById(String businessId, boolean isVisited) {
		try {
			...
			if (rs.next()) {
				...
				//shun xu yao he Restaurant class lei shengming shunxu yi yang
